...
const navLinkSx = {
  ...
  textDecoration: 'none',
  fontSize: 24,
  color: (theme: Theme) => theme.textColor.text3,
  padding: '13px 24px',
  width: '100%',
  textAlign: 'left',
  display: 'flex',
    ...
    <>
      ...
      <ShowOnMobile breakpoint="md">
        <StyledAppBar>
          ...
          {isOpen ? (
            <TextButton onClick={handleDismiss}>{<Close sx={{ fill: '#000' }} />}</TextButton>
          ) : (
            <TextButton onClick={handleClick}>{<Menu sx={{ fill: '#000' }} />}</TextButton>
          )}
        </StyledAppBar>
      </ShowOnMobile>
    </>
